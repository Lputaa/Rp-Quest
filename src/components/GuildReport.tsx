...
export default function GuildReport({ transactions, profile }: { transactions: Transaction[], profile: UserProfile }) {
  ...
  const diffPercentage = savedLast === 0 ? 0 : (diff / Math.abs(savedLast)) * 100;

  return (
    <div className="bg-[#3e2723] border-4 border-black p-6 shadow-[8px_8px_0_0_#000] text-white my-8 max-w-2xl mx-auto">
      <h2 className="font-display text-2xl md:text-3xl text-[#ffcc00] uppercase text-center mb-2 tracking-tighter" style={{ textShadow: "2px 2px 0px #000" }}>
      ...
      {!opened ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="font-sans font-bold uppercase text-[#ffcc00] animate-pulse">
        ...
        <motion.div 
          ...
          className="bg-[#1a1a17] border-4 border-black p-6 shadow-[inset_4px_4px_0_rgba(0,0,0,0.5)]"
        >
           <div className="flex justify-center mb-6 text-6xl drop-shadow-[0_0_20px_rgba(255,204,0,0.8)] animate-bounce">
              {savedCurrent >= 0 ? '🏆' : '💀'}
           </div>
           ...
           <div className="text-center mb-8 border-b-2 border-[#5d4037] pb-6">
              ...
              <p className={`font-display text-4xl md:text-5xl ${savedCurrent >= 0 ? 'text-[#ffcc00]' : 'text-red-500'}`}>
                {savedCurrent >= 0 ? '+' : ''} Rp {savedCurrent.toLocaleString('id-ID')}
              </p>
           </div>

           <div className="grid grid-cols-2 gap-4 text-center font-sans text-xs md:text-sm">
             <div className="border-r-2 border-[#5d4037] pr-4">
                <p className="text-gray-400 uppercase mb-1">{language === 'id' ? 'Bulan Lalu' : 'Last Month'}</p>
                <p className="font-bold text-[#f4e4bc]">Rp {savedLast.toLocaleString('id-ID')}</p>
             </div>
             <div className="pl-4">
                <p className="text-gray-400 uppercase mb-1">{language === 'id' ? 'Pertumbuhan' : 'Growth'}</p>
                <div className="flex items-center justify-center gap-1">
                  <span className={`font-bold ${diff > 0 ? 'text-green-500' : diff < 0 ? 'text-red-500' : 'text-gray-300'}`}>
                     {diff > 0 ? '↑' : diff < 0 ? '↓' : '-'} {Math.abs(diffPercentage).toFixed(1)}%
                  </span>
                </div>
             </div>
           </div>
