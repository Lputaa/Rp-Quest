...
import SageObjective from "./SageObjective";
...
const ForbiddenSeal = lazy(() => import("./ForbiddenSeal"));
...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "quest" | "oracle" | "report" | "calendar" | "seal" | "settings"
  >("quest");
  ...
  useEffect(() => {
    ...
    const qEvents = query(collection(db, "users", uid, "scheduledEvents"));
  ...
  return (
    <div className="max-w-[1200px] w-full mx-auto p-4 md:p-6 flex flex-col gap-6 h-full min-h-screen relative">
      <AnimatePresence>
        {decrees.length > 0 && (
          <div className="fixed inset-x-0 top-[25%] md:top-[25%] z-50 px-4 md:max-w-md md:mx-auto md:right-8 md:inset-x-auto w-full flex flex-col gap-2 pointer-events-none">
            {decrees.map((d) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="font-sans text-sm font-bold text-[#3e2723]">
                    ...
                    {isMissed && (
                      <span className="text-red-700">
                        Has not been recorded. Record it now?
                      </span>
                    )}
                    {isToday && (
                      <span>
                        Is due today! Rp {d.amount.toLocaleString("id-ID")}
                      </span>
                    )}
                    {isTomorrow && (
                      <span>
                        Heads up! Due tomorrow: Rp{" "}
                  ...
                  {(isMissed || isToday) && (
                    <button
                      onClick={() => handleManualLog(d)}
                      className="mt-3 w-full bg-[#ffcc00] border-2 border-black py-2 font-bold uppercase text-xs hover:bg-[#ffe066] flex justify-center items-center gap-1 shadow-[2px_2px_0_0_#000]"
                    >
      ...
      <header className="flex flex-col mb-4 lg:mb-8 border-b-4 border-[#3d251e] pb-4 gap-4 lg:gap-6 mt-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 w-full">
          <div className="w-full lg:w-auto">
            {profile && (
              <HeroStats transactions={transactions} profile={profile} />
            )}
          </div>
              ...
              <div className="relative w-full lg:w-max">
                ...
                >
                  {[
                    ...
                    {
                      id: "report",
                      label: language === "id" ? "Rapor Misi" : "Guild Report",
                    },
      ...
      {isNewAccount ? (
        <div className="flex-1 flex flex-col items-center justify-center p-6 border-4 border-black bg-[#1a1a17] text-white shadow-[8px_8px_0_0_#000] text-center my-8 mx-auto w-full max-w-2xl">
          {!showStarterInput ? (
            <>
              <h2 className="font-display text-xl sm:text-2xl md:text-4xl mb-4 text-[#ffcc00] uppercase break-words">
                {language === "id"
                  ? "Mulai Petualanganmu!"
              ...
              <p className="font-sans text-[#f4e4bc] mb-8 max-w-md leading-relaxed">
                {language === "id" ? (
                  <>
                    Sebelum memulai misi, tentukan terlebih dahulu target
                    pengeluaran harian maksimalmu. Ini akan menjadi batas{" "}
                    <strong className="text-red-500">HP-mu</strong> dalam
                    bertahan hidup di dunia nyata!
                  </>
                ) : (
                  <>
                    Before starting your mission, define your maximum daily
                    expense target. This will act as your{" "}
                    <strong className="text-red-500">HP</strong> limit for
            ...
            <form
              onSubmit={handleStartAdventure}
              className="w-full max-w-sm flex flex-col gap-4 mx-auto animate-in fade-in slide-in-from-bottom-4"
            >
              <label
                htmlFor="starterTarget"
                className="font-sans font-bold text-[#f4e4bc] mb-1 uppercase tracking-widest text-sm text-left"
              >
                {language === "id"
                  ? "Target Pengeluaran Harian (Max HP)"
                  : "Daily Expense Target (Max HP)"}
              </label>
              <input
                id="starterTarget"
                type="number"
                ...
                placeholder={
                  language === "id" ? "Contoh: 150000" : "E.g., 150000"
                }
                autoFocus
              />
        ...
        <>
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="col-span-1 lg:hidden flex flex-col order-2 -mx-4">
                ...
                >
                  ...
                  <div className="w-[85vw] sm:w-[350px] flex-shrink-0 snap-start hide-scroll">
                    <SageObjective transactions={transactions} />
                  </div>
                  <div className="w-[85vw] sm:w-[350px] flex-shrink-0 snap-start hide-scroll">
          ...
          {activeTab === "oracle" && (
            <Suspense
              fallback={
                <div className="p-8 text-center text-[#ffcc00] animate-pulse">
          ...
          {activeTab === "seal" && (
            <Suspense
              fallback={
                <div className="p-8 text-center text-red-500 animate-pulse">
                  Breaking Seals...
                </div>
              }
            >
